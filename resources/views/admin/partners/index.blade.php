...
<header class="mb-4 text-right">
    <a href="{{ route('admin.partners.create') }}" class="cursor-pointer inline-flex justify-center items-center gap-2 px-6 py-3 bg-amber-600 text-white rounded-2xl font-bold shadow-lg shadow-amber-100 hover:bg-amber-700 active:scale-95 transition">
...
<div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
    <div class="px-8 py-6 bg-slate-50/50 border-b gap-4 w-full">
        <form action="{{ route('admin.partners.index') }}" method="GET" class="flex w-full gap-2">
            <input type="text" name="search" placeholder="Cari nama partner..." class="flex-1 px-5 py-3 rounded-xl border-slate-200 border bg-white focus:ring-2 focus:ring-amber-500 outline-none transition">
                ...
                <tr class="hover:bg-slate-50/50 transition">
                    ...
                    <td class="px-8 py-6">
                        <div class="flex gap-2">
                            <a href="{{ route('admin.partners.edit', $partner->id) }}" class="cursor-pointer p-2.5 bg-amber-50 text-amber-600 rounded-xl hover:bg-amber-600 hover:text-white transition">
