...
<section class="max-w-7xl mx-auto px-6 py-20 flex flex-col md:flex-row items-center gap-12">
    <div class="flex-1 space-y-8">
        ...
        <div class="flex gap-4">
            <a href="{{ route('events.index') }}" class="px-8 py-4 bg-indigo-600 text-white rounded-2xl font-bold text-lg shadow-xl shadow-indigo-200 hover:scale-105 transition-transform">
                Mulai Jelajah
            </a>
...
<section id="events" class="max-w-7xl mx-auto px-6 py-20">
    <div class="flex flex-col lg:flex-row lg:justify-between items-start gap-4 lg:gap-0 mb-12">
        ...
        <div class="mb-8 flex gap-4 justify-center">
            <a href="/" class="px-4 py-2 rounded shadow-sm transition font-medium {{ !request('category') ? 'bg-indigo-600 text-white' : 'bg-indigo-100 hover:bg-indigo-200 text-indigo-700' }}">
                Semua Kategori
            </a>
            @foreach ($categories->take(2) as $category)
            <a href="/?category={{ $category->slug }}" class="px-4 py-2 rounded shadow-sm transition font-medium {{ request('category') === $category->slug ? 'bg-indigo-600 text-white' : 'bg-indigo-100 hover:bg-indigo-200 text-indigo-700' }}">
                {{ $category->name }}
            </a>
            @endforeach
        </div>
